...
@interface MySalesController ()<dropdownDelegate,UITableViewDataSource,UITableViewDelegate>{
...
@implementation MySalesController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title=@"我的销售";
...
-(UITableViewCell *)tableView:(UITableView *)tableView1 cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.num.text=[NSString stringWithFormat:@"%@:%@",@"销售台数",[dic objectForKey:@"num"]];
...
- (void)dropdownSelectedButtonIndex:(NSString *)index LeftIndex:(NSString *)left RightIndex:(NSString *)right {
    ...
    if (![self.organSelected isEqualToString:@""] && ![self.yearSelected isEqualToString:@""])
    {
        ...
        [HttpRequest getWithURLString:@"http://221.130.108.114:8082/XCAPI/salesofstaff.ashx" parameters:dic success:^(id responseObject) {
